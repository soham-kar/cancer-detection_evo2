...
import { Check, X, Shield, ExternalLink, Download, Copy, Trash2, Brain, FileDown } from "lucide-react";
...
function computeConfidenceFactors(report: SavedReport) {
    ...
    const factors: { label: string; contribution: number; color: string; detail: string }[] = [];
    ...
    const deltaContrib = Math.min(50, Math.round((Math.log10(absDelta / EVO2_WEAK + 1) / Math.log10(EVO2_MAX / EVO2_WEAK + 1)) * 50));
    const signalLabel = absDelta >= EVO2_STRONG ? "strong" : absDelta >= EVO2_MOD ? "moderate" : absDelta >= EVO2_WEAK ? "weak" : "minimal";
    factors.push({
    ...
    const directionContrib = nearZero ? 5 : ambiguous ? 10 : absDelta >= EVO2_STRONG ? 20 : 14;
    const directionLabel = nearZero ? `Effect Size: Minimal` : ambiguous ? `Effect Size: Weak` : `Effect Size: Clear`;
    factors.push({
    ...
    let popContrib = 10;
    let popDetail = "Not observed in gnomAD (800k+ individuals) — novel or ultra-rare";
    if (af !== null && af !== undefined) {
        if (af > 0.05) { popContrib = 2; popDetail = `Common variant (AF=${(af * 100).toFixed(2)}%) — strong benign signal (BA1 criterion)`; }
        else if (af > 0.01) { popContrib = 5; popDetail = `Uncommon (AF=${(af * 100).toFixed(2)}%)`; }
        else if (af > 0.001) { popContrib = 10; popDetail = `Rare (AF=${(af * 100).toFixed(4)}%) — low population frequency`; }
        else { popContrib = 15; popDetail = `Ultra-rare (AF=${af.toExponential(2)}) — rarity supports pathogenic classification`; }
    }
    factors.push({
    ...
    if (acmgCode && acmgCode !== "None") {
        if (acmgCode.includes("VeryStrong") || acmgCode.includes("PVS")) { acmgContrib = 15; acmgDetail = `${acmgCode} — very strong ACMG criterion met`; }
        else if (acmgCode.includes("Strong") || acmgCode.startsWith("PS") || acmgCode.startsWith("BS")) { acmgContrib = 12; acmgDetail = `${acmgCode} — strong ACMG criterion`; }
        else if (acmgCode.includes("Moderate") || acmgCode.startsWith("PM") || acmgCode.startsWith("BP")) { acmgContrib = 9; acmgDetail = `${acmgCode} — moderate ACMG criterion`; }
        else if (acmgCode.includes("Supporting") || acmgCode.startsWith("PP")) { acmgContrib = 6; acmgDetail = `${acmgCode} — supporting ACMG criterion`; }
        else { acmgContrib = 7; acmgDetail = `${acmgCode} — ACMG criterion triggered`; }
    }
    factors.push({
        label: "ACMG Evidence",
...
export function SavedReportModal({
    ...
    const confidencePct = Math.round((report.classificationConfidence ?? 0) * 100);
    ...
    const acmgDetail = report.acmgEvidence?.clinical_note
    ...
    const generateReportText = () => {
        ...
        const varSomeChrom = report.chromosome.startsWith("chr") ? report.chromosome : `chr${report.chromosome}`;
...
${(report.literatureContext?.pubmed_ids ?? []).slice(0, 5).map(id => `• https://pubmed.ncbi.nlm.nih.gov/${id}/`).join("\n") || "• No references available"}
...
`;
    ...
    const copyReport = async () => {
        try { await navigator.clipboard.writeText(generateReportText()); showSuccessToast("Report copied!", "📋"); }
        catch { showSuccessToast("Failed to copy", "❌"); }
    };
    ...
    const downloadPDF = async () => {
        ...
        try {
            ...
            window.getComputedStyle = (element: Element, pseudoElt?: string | null): CSSStyleDeclaration => {
                ...
                return new Proxy(cs, {
                    get(target, prop: string | symbol) {
                        ...
                        if (typeof raw === "function") return (raw as Function).bind(target);
                        return raw;
                    }
                });
            };
            ...
            const addHeader = (pdf: InstanceType<typeof jsPDF>, pageNum: number, totalPages: number) => {
                pdf.setFontSize(8);
                ...
                pdf.text(`Page ${pageNum} of ${totalPages}`, pageW - margin, 8, { align: "right" });
            ...
            for (let page = 1; page <= totalPages; page++) {
                ...
                pdf.addImage(pageImg, "PNG", margin, margin + 12, contentW, sliceRenderH, undefined, "FAST");
            ...
            pdf.save(`helixmind-${report.geneSymbol}-${report.chromosome}-${report.position}.pdf`);
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            if (response.ok) { onDelete?.(report.id); onClose(); }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 overflow-y-auto h-screen min-h-screen">
            <div className="max-h-[90vh] w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-xl">
                ...
                {/* Header */}
                <div className="border-b border-[#3c4f3d]/10 p-5 flex items-center justify-between">
                    <h3 className="text-lg font-medium text-[#3c4f3d]">Variant Analysis Report</h3>
                ...
                {/* Scrollable content */}
                <div ref={contentRef} className="max-h-[calc(90vh-130px)] overflow-y-auto p-5 space-y-5">
                    ...
                    <div className="rounded-md border border-[#3c4f3d]/10 bg-[#e9eeea]/30 p-4">
                        ...
                        <div className="grid gap-4 md:grid-cols-2 text-xs">
                            <div className="space-y-2">
                                <div className="flex"><span className="w-28 text-[#3c4f3d]/70">Position:</span><span>{report.position.toLocaleString()}</span></div>
                                <div className="flex"><span className="w-28 text-[#3c4f3d]/70">Type:</span><span>{report.variationType || "Single Nucleotide Variant"}</span></div>
                            </div>
                            <div className="space-y-2">
                                <div className="flex">
                                ...
                                <div className="flex"><span className="w-28 text-[#3c4f3d]/70">Gene:</span><span className="font-medium">{report.geneSymbol}</span></div>
                            </div>
                        </div>
                        <div className="mt-4 flex flex-wrap gap-3">
                            <a href={report.clinvarId
                            ...
                            <a href={`https://varsome.com/position/${report.genomeId}/${varSomeChrom}-${report.position}`}
                                target="_blank" rel="noopener noreferrer"
                            ...
                            <a href={`https://gnomad.broadinstitute.org/variant/${report.chromosome.replace("chr", "")}-${report.position}-${report.reference || "N"}-${report.alternative}`}
                                target="_blank" rel="noopener noreferrer"
                                className="inline-flex items-center gap-1.5 rounded-md bg-blue-500/10 px-3 py-1.5 text-xs font-medium text-blue-700 hover:bg-blue-500/20 transition-colors">
                        ...
                        <div className="rounded-md border border-[#3c4f3d]/10 bg-white p-4">
                            <div className="grid gap-4 md:grid-cols-2">
                                ...
                                <div className="rounded-md bg-[#e9eeea]/50 p-4">
                                    ...
                                    <div className="mt-3">
                                        <div className="mb-1 text-xs text-[#3c4f3d]/70">Delta Likelihood Score:</div>
                                        <div className={`text-sm font-semibold font-mono ${isPathogenic ? "text-red-800" : isBenign ? "text-green-800" : "text-[#3c4f3d]"}`}>{delta.toFixed(6)}</div>
                                        <div className="mt-2 relative h-3 rounded-full bg-gradient-to-r from-red-500 via-yellow-400 to-green-500">
                                            <div className="absolute top-0 left-1/2 h-3 w-0.5 bg-white/70"></div>
                                            <div className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 h-4 w-4 rounded-full border-2 border-white bg-[#3c4f3d] shadow-md"
                                                style={{ left: `${Math.max(5, Math.min(95, ((Math.max(-0.01, Math.min(0.01, delta)) + 0.01) / 0.02) * 100))}%` }} />
                                        </div>
                                        <div className="mt-1 flex justify-between text-[10px] text-[#3c4f3d]/60">
                                            <span>Pathogenic</span><span>Neutral</span><span>Benign</span>
                                        </div>
                                    </div>
                                    <div className="mt-3">
                                        <div className="mb-1 text-xs text-[#3c4f3d]/70">Confidence:</div>
                                        <div className="h-2 w-full rounded-full bg-gray-200 overflow-hidden">
                                            <div className="h-full rounded-full bg-gradient-to-r from-green-400 to-green-600"
                                                style={{ width: `${confidencePct}%` }} />
                                        </div>
                                        <div className="mt-1 text-right text-xs font-medium text-[#3c4f3d]">{confidencePct}%</div>
                                    </div>
                                </div>
                            </div>
                            {isDiscordant && (
                                <div className="mt-4 flex items-center gap-2 text-xs text-amber-700">
                                    <span className="flex h-4 w-4 items-center justify-center text-amber-600 font-bold">!</span>
                                    <span>ClinVar ({report.clinvarClassification}) differs from Evo2 ({report.prediction})</span>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="rounded-md border border-[#3c4f3d]/10 bg-[#e9eeea]/30 p-4">
                        <h4 className="mb-3 text-sm font-medium text-[#3c4f3d]">Clinical Annotations</h4>
                        <div className="grid gap-4 md:grid-cols-3">
                            <div className="rounded-md bg-white p-3">
                                <div className="text-xs font-medium text-[#3c4f3d]/70 mb-1">Population Frequency</div>
                                {af !== null && af !== undefined
                                    ? <div className="text-sm text-[#3c4f3d] font-mono">{(af * 100).toFixed(6)}%</div>
                                    : <div className="text-sm text-[#3c4f3d]/50">Not found in gnomAD</div>}
                                {report.populationFrequency?.frequency_classification && (
                                    <div className="mt-1 text-xs text-green-700 font-medium">{report.populationFrequency.frequency_classification}</div>
                                )}
                            </div>
                            <div className="rounded-md bg-white p-3">
                                <div className="text-xs font-medium text-[#3c4f3d]/70 mb-1">ACMG Evidence</div>
                                {report.acmgEvidence?.code && report.acmgEvidence.code !== "None"
                                    ? <div className="text-xs font-mono text-indigo-700 font-semibold mb-1">{report.acmgEvidence.code}</div>
                                    : null}
                                <div className="text-xs text-[#3c4f3d]/60">{report.acmgEvidence?.description || "Computational evidence is inconclusive"}</div>
                            </div>
                            <div className="rounded-md bg-white p-3">
                                <div className="text-xs font-medium text-[#3c4f3d]/70 mb-1">Literature</div>
                                <div className="text-sm text-[#3c4f3d]/60 mb-2">
                                    {(report.literatureContext?.articles_found ?? 0) > 0
                                        ...
                                        : <span className="text-amber-600">0 articles <span className="text-[10px]">(PubMed unavailable)</span></span>}
                                </div>
                                {(report.literatureContext?.pubmed_ids ?? []).length > 0 && (
                                    <div className="flex flex-wrap gap-1">
                                        {report.literatureContext!.pubmed_ids.slice(0, 5).map((id) => (
                                            <a key={id} href={`https://pubmed.ncbi.nlm.nih.gov/${id}/`}
                                                target="_blank" rel="noopener noreferrer"
                                                className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-medium rounded-full border bg-slate-100 text-slate-600 border-slate-200 hover:bg-slate-200 transition-colors">
                                                PMID:{id}
                                            </a>
                                        ))}
                                    </div>
                ...
                <div className="border-t border-[#3c4f3d]/10 bg-[#f9fafb] px-5 py-3 flex flex-wrap items-center justify-between gap-2 rounded-b-2xl">
                    <div className="flex items-center gap-2">
                        <button
                            onClick={copyReport}
                            className="inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium text-[#3c4f3d]/70 hover:bg-[#3c4f3d]/10 transition-colors"
                        >
                        ...
                        <button
                            ...
                            className="inline-flex items-center gap-1.5 rounded-md bg-indigo-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-indigo-700 disabled:opacity-60 disabled:cursor-not-allowed transition-colors shadow-sm"
                        >
                    ...
                    <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium text-red-500 hover:bg-red-50 transition-colors"
                    >
            ...
            {
                successToast && (
                    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-2 rounded-full bg-[#3c4f3d] px-4 py-2 text-sm text-white shadow-lg animate-in fade-in slide-in-from-bottom-2">
                        <span>{successToast.icon}</span><span>{successToast.message}</span>
                    </div>
                )
            }
            ...
            {/* Delete confirm */}
            {
                showDeleteConfirm && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40">
                        <div className="bg-white rounded-xl p-5 shadow-xl max-w-xs w-full mx-4">
                            ...
                            <div className="flex gap-2 justify-end">
                                ...
                                <button
                                    ...
                                    disabled={isDeleting}
                                    className="px-3 py-1.5 text-sm font-medium text-white bg-red-500 rounded-md hover:bg-red-600 disabled:opacity-60 transition-colors"
                                >
