...
function getClassificationColor(classification: string): string {
  ...
  if (lower.includes("pathogenic")) return "text-red-600 bg-red-50 border-red-200";
  if (lower.includes("benign")) return "text-green-600 bg-green-50 border-green-200";
...
function getAgreementBadge(level: string, modelsAgree: number, modelsTotal: number): {
  label: string;
  color: string;
} {
  switch (level) {
    case "Full":
      return { label: `${modelsAgree}/${modelsTotal} Full Agreement`, color: "bg-emerald-100 text-emerald-700" };
    case "Strong":
      return { label: `${modelsAgree}/${modelsTotal} Strong Consensus`, color: "bg-blue-100 text-blue-700" };
    case "Split":
      return { label: `${modelsAgree}/${modelsTotal} Split Decision`, color: "bg-amber-100 text-amber-700" };
    case "Weak":
      return { label: `${modelsAgree}/${modelsTotal} Weak Consensus`, color: "bg-red-100 text-red-700" };
    default:
      return { label: `${modelsAgree}/${modelsTotal} Models`, color: "bg-slate-100 text-slate-600" };
  }
}
...
export function MultiModelConsensus({ data, className = "" }: MultiModelConsensusProps) {
  if (!data || !data.predictions || data.predictions.length === 0) {
    return null;
  }

  const agreement = getAgreementBadge(data.agreement_level, data.models_agree, data.models_total);
    ...
    <div className={`rounded-xl border border-slate-200 bg-white overflow-hidden ${className}`}>
      ...
      <div className="px-4 py-3 border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-purple-50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-slate-700">Multi-Model Consensus</h3>
          </div>
          <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${agreement.color}`}>
            {agreement.label}
          </span>
        </div>
      </div>
      ...
      <div className="px-4 py-3 border-b border-slate-100">
        <div className="flex items-center gap-3">
          <div className={`px-3 py-1.5 rounded-lg border text-sm font-bold ${consensusColor}`}>
            {data.consensus_classification}
          </div>
          <div className="flex items-center gap-1.5">
            <div className={`w-2 h-2 rounded-full ${getConfidenceColor(data.consensus_confidence)}`} />
            <span className="text-xs text-slate-500">
              {data.consensus_confidence} confidence
            </span>
          </div>
        </div>
        <p className="text-xs text-slate-500 mt-1.5">{data.clinical_note}</p>
      </div>
        ...
        {data.predictions.map((pred) => (
          <div key={pred.model} className="flex items-center gap-3">
            ...
            <div className="w-32 flex items-center gap-1.5 flex-shrink-0">
              ...
              <span className="text-xs font-medium text-slate-600">{pred.model}</span>
            </div>
            ...
            <span
              className={`text-[10px] px-1.5 py-0.5 rounded border font-medium flex-shrink-0 ${getClassificationColor(pred.classification)}`}
            >
              {pred.classification}
            </span>
            ...
            {/* Confidence dot */}
            <div
              className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${getConfidenceColor(pred.confidence)}`}
...
export function buildConsensusData(apiResult: {
  ...
  external_scores?: {
    alphamissense?: { score: number; confidence: string; classification: string };
    cadd?: { phred: number; interpretation: string };
    revel?: { score: number };
  };
}): ConsensusData | null {
  ...
  if (scores.cadd) {
    predictions.push({
      ...
      confidence: scores.cadd.phred >= 20 || scores.cadd.phred < 10 ? "High" : "Medium",
      weight: 0.2,
  ...
  if (scores.revel) {
    ...
    predictions.push({
      model: "REVEL",
      score: rScore,
      classification: rScore > 0.75 ? "Likely Pathogenic" : rScore < 0.25 ? "Likely Benign" : "Uncertain",
  ...
  // Compute consensus
  const pathogenicCount = predictions.filter((p) => p.classification.includes("Pathogenic")).length;
  const benignCount = predictions.filter((p) => p.classification.includes("Benign")).length;
  ...
  const consensusConfidence = agreeRatio >= 0.75 ? "High" : agreeRatio >= 0.5 ? "Medium" : "Low";
