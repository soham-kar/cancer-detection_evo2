...
\node[embed=evo2mag, below=0.3cm of evo2m, xshift=-0.7cm] (evo2e) 
    {$e_1$ present};
\node[embed=evo2mag, below=0.3cm of evo2m, xshift=+0.7cm] (evo2miss) 
    {$e_1$ missing};

\node[network, fill=evo2mag!8, draw=evo2mag, below=0.4cm of evo2e, xshift=0.7cm, minimum width=2.0cm] (evo2cat) 
    {concat};

\draw[arrow] (evo2s.east) -- ++(0.25,0) |- (evo2cat.west);
\draw[arrow] (evo2m.east) -- ++(0.25,0) |- (evo2cat.west);
\draw[arrow] (evo2e.east) -- ++(0.15,0) |- ([yshift=0.08cm]evo2cat.west);
\draw[dasharrow] (evo2miss.east) -- ++(0.15,0) |- ([yshift=-0.08cm]evo2cat.west);

% --- AM branch (right) ---
\node[input, fill=amorange!10, draw=amorange, right=3.0cm of evo2s] (ams) 
    {\textbf{AlphaMissense}\\$s_2 \in [0,1]$};
\node[input, fill=amorange!10, draw=amorange, below=0.2cm of ams] (amm) 
    {Mask $m_2$};

\node[embed=amorange, below=0.3cm of amm, xshift=-0.7cm] (ame) 
    {$e_2$ present};
\node[embed=amorange, below=0.3cm of amm, xshift=+0.7cm] (ammiss) 
    {$e_2$ missing};

\node[network, fill=amorange!8, draw=amorange, below=0.4cm of ame, xshift=0.7cm, minimum width=2.0cm] (amcat) 
    {concat};

\draw[arrow] (ams.east) -- ++(0.25,0) |- (amcat.west);
\draw[arrow] (amm.east) -- ++(0.25,0) |- (amcat.west);
\draw[arrow] (ame.east) -- ++(0.15,0) |- ([yshift=0.08cm]amcat.west);
\draw[dasharrow] (ammiss.east) -- ++(0.15,0) |- ([yshift=-0.08cm]amcat.west);

% Background box
\scoped[on background layer]{
    \node[fit=(evo2e)(ammiss), draw=gray, dashed, fill=gray!5,
          inner sep=5pt, rounded corners=3pt] (embed_region) {};
    \node[annot, above=0.03cm of embed_region.north] {Predictor‑specific missing embeddings};
}

% ==================================================================
% ROW 2: DEEP SETS (centered below)
% ==================================================================
\node[network, fill=evidencepurple!10, draw=evidencepurple, below=0.8cm of evo2cat, xshift=1.5cm, minimum width=4.5cm] (deepset_phi) 
    {$\varphi$ MLP (shared)};
...
\draw[arrow] (evo2cat.south) -- ++(0,-0.2) -| ([xshift=-0.6cm]deepset_phi.north);
\draw[arrow] (amcat.south) -- ++(0,-0.2) -| ([xshift=0.6cm]deepset_phi.north);
...
% ==================================================================
\node[input, fill=gray!12, draw=gray, left=2.0cm of deepset_phi.west, yshift=-0.4cm] (vartype) 
...
% PARAMETER COUNT
% ==================================================================
\node[annot, below=0.08cm of deepset_rho.south, text=dark] 
