"""
OPTIMIZED Bayesian Framework - Fast & Production-Ready

Fixes:
1. Reduced Monte Carlo samples (100 instead of 1000) for speed
2. Proper AF column handling (af_nfe vs af_eur)
3. Validation checks built-in
4. Works with pre-scored data (faster than re-scoring)
"""
...
def monte_carlo_credible_interval_fast(evo2_score: float, af_values: list,
                                       n_samples: int = BayesianConfig.N_MC_SAMPLES) -> tuple:
    """OPTIMIZED Monte Carlo with 100 samples (10x faster)"""
    posterior_samples = []
    
    for _ in range(n_samples):
        # Sample k
        k_sample = max(np.random.normal(BayesianConfig.EVO2_SCALE, 
                                        BayesianConfig.EVO2_SCALE_STD), 5)
...
def process_variants_optimized(input_file: str, output_dir: Path):
    """FAST processing with pre-scored data"""
    
    print("="*80)
    print("OPTIMIZED BAYESIAN FRAMEWORK - BALANCED & FAST")
    print("="*80)
    
    df = pd.read_csv(input_file)
    print(f"\n📊 Loaded {len(df)} variants")
    
    # FIXED: Handle both af_nfe and af_eur column names
    af_col_mapping = {
        'af_nfe': 'af_eur',  # Map nfe to eur for consistency
    }
    
    # Check which AF columns exist
    af_cols = []
    for col in ['af_afr', 'af_nfe', 'af_eas', 'af_sas', 'af_amr']:
        if col in df.columns:
            af_cols.append(col)
        elif col == 'af_nfe' and 'af_eur' in df.columns:
            df['af_nfe'] = df['af_eur']  # Create alias
            af_cols.append('af_nfe')
    
    print(f"   Found AF columns: {af_cols}")
    
    # Check for Evo2 scores
    if 'evo2_score' not in df.columns:
        print("❌ Missing evo2_score column. Need pre-scored data.")
        return None
    
    # Process
    print(f"\n⚙️  Computing with optimized Monte Carlo (n={BayesianConfig.N_MC_SAMPLES} samples/variant)...")
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing"):
        ...
        # Fast Monte Carlo (100 samples)
        ci_lower, ci_upper, samples = monte_carlo_credible_interval_fast(evo2_score, af_values)
    ...
    # Save
    output_file = output_dir / "brca1_bayesian_optimized.csv"
...
def run_validation_checks(df: pd.DataFrame):
    ...
    # Check 1: CI width range
    mean_ci = df['ci_width'].mean()
    print(f"\n1. Mean CI width: {mean_ci:.4f}")
    ...
    print(f"   Expected: 10-35%")
    if 0.10 < vus_rate < 0.35:
        print("   ✅ PASS")
        checks_passed += 1
    else:
        print(f"   ❌ FAIL")
    
    # Check 3: AF=0 uncertainty
    af_zero = df[df.get('af_nfe', df.get('af_eur', 0)) == 0]
    if len(af_zero) > 0:
        median_ci_zero = af_zero['ci_width'].median()
        print(f"\n3. AF=0 median CI width: {median_ci_zero:.4f}")
        print(f"   Expected: >0.01")
        if median_ci_zero > 0.01:
            print("   ✅ PASS")
            checks_passed += 1
        else:
            print("   ❌ FAIL")
    else:
        print("\n3. No AF=0 variants - SKIP")
        checks_total -= 1
    
    # Check 4: Posterior spread
    post_spread = df['posterior'].between(0.01, 0.99).mean()
    print(f"\n4. Posterior spread: {post_spread*100:.1f}% in (0.01, 0.99)")
    print(f"   Expected: >80%")
    if post_spread > 0.80:
        print("   ✅ PASS")
        checks_passed += 1
    ...
    # Check 5: Strong Evo2 overrides prior
    strong_path = df[df['evo2_score'] < -0.05]
    if len(strong_path) > 10:
        median_post_strong = strong_path['posterior'].median()
        print(f"\n5. Strong pathogenic (score<-0.05) median posterior: {median_post_strong:.4f}")
        print(f"   Expected: >0.5")
        if median_post_strong > 0.5:
            print("   ✅ PASS")
            checks_passed += 1
        else:
            print("   ❌ FAIL")
    else:
        print("\n5. Not enough strong pathogenic variants - SKIP")
        checks_total -= 1
    
    print("\n" + "="*80)
    print(f"FINAL SCORE: {checks_passed}/{checks_total} checks passed")
    if checks_passed == checks_total:
        print("✅ ALL CHECKS PASSED - Framework is balanced!")
    else:
        print(f"⚠️  {checks_total - checks_passed} check(s) failed - Review parameters")
    print("="*80)
...
def create_diagnostic_plots(df: pd.DataFrame, output_dir: Path):
    ...
    if af_col in df.columns:
        mask = df[af_col] > 0
        ax.scatter(df.loc[mask, af_col], df.loc[mask, 'posterior'], alpha=0.4, s=15)
        ax.set_xscale('log')
        ax.set_xlabel('gnomAD AF', fontweight='bold')
    ...
    ax = axes[0, 1]
    ax.hist(df['ci_width'], bins=40, color='#3498db', alpha=0.7, edgecolor='black', linewidth=0.5)
    ax.axvline(df['ci_width'].mean(), color='red', linestyle='--', linewidth=2,
               label=f'Mean: {df["ci_width"].mean():.3f}')
    ax.axvline(0.15, color='orange', linestyle='--', linewidth=2, label='Confidence threshold')
    ...
    ax.hist(bf_log, bins=40, color='#e74c3c', alpha=0.7, edgecolor='black', linewidth=0.5)
    ax.axvline(0, color='black', linestyle='--', linewidth=2, label='BF=1 (neutral)')
    ...
    output_path = output_dir / "diagnostic_validation.png"
...
def main():
    ...
    output_dir = Path("results/bayesian_optimized")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Process
    results_df = process_variants_optimized(input_file, output_dir)
    
    if results_df is not None:
        # Validation
        run_validation_checks(results_df)
        ...
        print("\n✅ OPTIMIZED BAYESIAN FRAMEWORK COMPLETE!")
